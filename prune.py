...
def prune_and_evaluate(prune_rate):
    ...
    fc2_weights = model.classifier[3].weight.data.cpu().numpy()
    blocks2 = fc2_weights.reshape(64, 64, 64, 64).swapaxes(1, 2).reshape(64, 64, 64, 64)


    # 生成掩码并进行剪枝
    mask = np.random.rand(16, 98) < prune_rate
    mask2 = np.random.rand(64, 64) < 0.7
    #print(mask)
    blocks[mask] = 0
    blocks2[mask2] = 0
    # 将修改后的参数矩阵重新赋值给模型
    fc1_weights = blocks.reshape(16, 98, 256, 256).swapaxes(1, 2).reshape(16, 256, 98, 256).reshape(4096, 25088)
    fc2_weights = blocks2.reshape(64, 64, 64, 64).swapaxes(1, 2).reshape(64, 64, 64, 64).reshape(4096, 4096)
    #print(fc1_weights)
    model.classifier[0].weight.data = torch.from_numpy(fc1_weights).to(device)
    model.classifier[3].weight.data = torch.from_numpy(fc2_weights).to(device)
